...
export const i18n: Record<'de' | 'en', I18nStrings> = {
  de: {
    ...
    runningOutro: 'Die ersten beiden können Sie sofort im Browser aufrufen. Bei der Rechnungsstrecke steht die Norm statt eines Links, weil der Kunde unter NDA liegt. Das Gateway ist offen, aber es ist keine Webseite: es spricht MCP, und anonym gibt es dort nur Auskunft.',
  ...
  en: {
    ...
    runningOutro: 'The first two you can open in a browser right now. For the invoicing pipeline the standard stands in place of a link, because the client is under NDA. The gateway is open too, but it is not a web page: it speaks MCP, and anonymous callers only get lookups.',
