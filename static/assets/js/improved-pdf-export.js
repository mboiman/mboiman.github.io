/**
 * Verbesserter PDF-Export für reveal.js Präsentationen
 * Löst Probleme mit Formatierung und mehrseitigen Folien
 * v1.1.0
 */
(function () {
  "use strict";

  // Logger-Funktion für besseres Debugging
  ...
  function initImprovedPdfExport() {
    log("Verbesserter PDF-Export wird initialisiert...");

    if (isPrintPdfMode()) {
      log("PDF-Export-Modus erkannt");
      prepareForImprovedPrinting();
    } else {
    ...
    if (typeof Reveal !== "undefined") {
      ...
      log("Reveal.js nicht gefunden!", "warn");
    }
  }
  ...
  function createPdfButton() {
    ...
    button.innerHTML = '<i class="fas fa-file-pdf"></i> PDF Export';
    button.style.cssText =
      "position: fixed; bottom: 20px; left: 20px; background-color: rgba(0,0,0,0.6); color: white; padding: 10px; border-radius: 5px; cursor: pointer; z-index: 999; font-size: 14px;";
  ...
  function handlePdfButtonClick(e) {
    ...
    showImprovedPdfDialog(pdfUrl);

    return false;
  }

  // Zeige verbesserten Dialog mit PDF-Export-Optionen
  function showImprovedPdfDialog(pdfUrl) {
    ...
    dialog.innerHTML = `
      ...
      <div style="display: flex; gap: 20px; margin: 20px 0; flex-wrap: wrap;">
        ...
        <div style="flex: 1; min-width: 240px; background-color: #f5f5f5; padding: 20px; border-radius: 5px; border-left: 4px solid #5cb85c;">
          ...
          <button id="export-pptx" style="width: 100%; padding: 10px; margin-top: 15px; background-color: #5cb85c; color: white; border: none; border-radius: 4px; cursor: pointer;">Als PPTX exportieren</button>
        </div>
      </div>
    ...
    document
      .getElementById("export-pptx")
      .addEventListener("click", function () {
        // PPTX-Export starten (hier implementieren wir später die PPTX-Export-Funktion)
        handlePptxExport();
      });
  ...
  // Handler für PPTX-Export
  function handlePptxExport() {
    // Zeige Lade-Dialog
    showLoadingDialog("Bereite PPTX-Export vor...");

    // Hier würde der tatsächliche PPTX-Export implementiert werden
    // Diese Funktion ist ein Platzhalter und würde in einer vollständigen Implementierung
    // mit Services wie pptxgenjs oder einem Server-basierten Konverter arbeiten

    // Für dieses Beispiel zeigen wir nur einen Hinweis-Dialog nach kurzer Verzögerung
    setTimeout(function () {
      removeLoadingDialog();
      showPptxInfo();
    }, 1500);
  ...
  // Zeigt einen Lade-Dialog
  function showLoadingDialog(message) {
    var loadingContainer = document.createElement("div");
    loadingContainer.id = "pdf-export-loading";
  ...
  // Entfernt den Lade-Dialog
  function removeLoadingDialog() {
    var loadingDialog = document.getElementById("pdf-export-loading");
    if (loadingDialog) {
      document.body.removeChild(loadingDialog);
    }
  }

  // Zeigt einen Info-Dialog für PPTX-Export
  function showPptxInfo() {
    ...
    infoContent.innerHTML = `
      ...
      <p>Die PPTX-Exportfunktion benötigt die Installation eines zusätzlichen Dienstes.</p>
      <p>Für die vollständige PPTX-Exportfunktion sind folgende Optionen verfügbar:</p>
      <ol>
        <li>Installation von <a href="https://github.com/astefanutti/decktape" target="_blank">DeckTape</a> für PDF-Erstellung und anschließende Konvertierung</li>
        <li>Verwendung einer servergestützten Lösung mit <a href="https://www.pptxgenjs.com/" target="_blank">pptxgenjs</a></li>
        <li>Integration mit einer Cloud-Konvertierungslösung</li>
      </ol>
      <p>Bitte kontaktieren Sie Ihren Administrator für die Einrichtung dieser Funktion.</p>
  ...
  // Vorbereitung für verbesserten PDF-Druck
  function prepareForImprovedPrinting() {
  ...
  // Lädt verbesserte CSS-Styles für den Druck
  function loadImprovedPrintStyles() {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    log("DOM vollständig geladen, initialisiere verbesserten PDF-Export...");
    initImprovedPdfExport();
  });
})();
